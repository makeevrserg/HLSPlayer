package com.makeevrserg.hlsplayer.utils.seekbar
...
//    var indicatorPositions: List<Float> = mutableListOf()
    var indicatorPositions:Map<Float,Int> = mutableMapOf()
    ...
    private fun drawIndicators(canvas: Canvas) {
        indicatorPositions.forEach {
            ...
            val barPositionCenter = (it.key/86399) * width()
    ...
    private fun updateProgress(event: MotionEvent): Boolean {
        ...
        val boundedPercent = Math.min(Math.max(percent, 0F), 1F) // not above 1
